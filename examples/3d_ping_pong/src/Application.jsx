...
import { Canvas, useFrame, useThree, useLoader, buildGraph } from "@react-three/fiber";
...
import pingpong from "../resources/pingpong.json";

function useGLTF_SHIM(resource) {
  return suspend(
    new Promise(resolve => {
      const loader = new GLTFLoader();
      loader.parse(resource, '../resources/', data => {
        if (data.scene) {
          // console.log('data', data);
          const _pingpong = Object.assign(data, buildGraph(data.scene));
          // console.log('pingpong', _pingpong);
          resolve(_pingpong);
        }
      });
    }),
...
const state = proxy({
  ...
  api: {
    pong(velocity) {
      ...
      if (velocity > 10) {
        ++state.count;
        const selector = '#current_score-tr > .data > span:nth-child(1) > input.x-form-text';
        const el = document.querySelector(selector);
        el.value = `${state.count}`;
      }
...
export default function Application({ ready }) {
  return (
...
function Paddle({ vec = new THREE.Vector3(), dir = new THREE.Vector3() }) {
  ...
  // const { nodes, materials } = useGLTF(pingpong);
  const { nodes, materials } = useGLTF_SHIM(pingpong);
